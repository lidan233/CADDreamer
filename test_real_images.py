...
from utils.graphcut import build_instance_graph_new_max
...
def save_images(mm1,   used_views, resolusion = 256, scales = None, color_size=14, line_thickness=2) -> None:
    ...
    path = os.path.join(OUTPUT_PATH, "./test"+ str(random.random())+".obj")
...
def process_final_connnected_components_objs(mesh, face_components, intial_objs,  label_components):
    ...
    assign_label_of_components = []
    temp_label_array = np.zeros(len(mesh.faces)) - 1
    for face_comp_idx, face_comp in enumerate(face_components):
        current_part_comps = list(nx.connected_components(real_mesh_face_graph.subgraph(face_comp)) )
        removed_current_part = max(current_part_comps, key=len)
        temp_label_array[list(removed_current_part)] = face_comp_idx
        if len(current_part_comps) > 1:
            current_part_comps.remove(removed_current_part)
            assign_label_of_components += current_part_comps
    
    start_comp_idx = temp_label_array.max() 
    for assign_comp_idx, assign_comp in enumerate(assign_label_of_components):
        temp_label_array[list(assign_comp)] = int(start_comp_idx + assign_comp_idx + 1)
    ee_list = temp_label_array[mesh.face_adjacency]
    comp_graph = nx.from_edgelist(ee_list.astype(np.int32))


    for assign_comp_idx, assign_comp in enumerate(assign_label_of_components):
        comp_idx = int(start_comp_idx + assign_comp_idx + 1)
        potential_neighrbos = [i for i in list(comp_graph.neighbors(comp_idx)) if i <= start_comp_idx]
        comp_vertices = mesh.vertices[mesh.faces[list(assign_comp)].reshape(-1)]
        project_distances = [new_initial_objs[neigh].batch_distance(comp_vertices).sum() for neigh in potential_neighrbos]
        choosed_label = potential_neighrbos[np.argmin(project_distances)]
        temp_label_array[list(assign_comp)] = choosed_label

    new_label_components = []
    for i in range(len(face_components)):
        new_label_components.append(np.where(temp_label_array==i)[0])
...
"""
--config_dir /mnt/disk/CADDreamer/cached_output/cropsize-256-cfg1.0/0_0deepcad --review False
"""
if __name__=='__main__':
    ...
    temp_path =  cfg.output_path + "/temp" + os.path.basename(cfg.config_dir)

    if not os.path.exists(temp_path) or cfg.review:
        with time_block("First step: multi-view normal and semantic maps "):
            result_masks = load_prediction_mask(os.path.dirname(cfg.config_dir), os.path.basename(cfg.config_dir))
            result_normals = load_prediction_normal(os.path.dirname(cfg.config_dir), os.path.basename(cfg.config_dir))

            extend_mask2instance = extract_mask_instance_mask(result_masks)
            ...
            bpy.data.cameras[0].ortho_scale = 1.15
            input_views = []
        ...
        with time_block("Second steps: get 3D partial instances from multi-view semantic maps"):
            ...
            pixel_to_faces_all, pixel_zbuf_all, project_face_centers_all, \
                mesh, camera_locations, zbuf_min, normal_colors_all, \
                pixel_to_faces_all_highresolu, zbuf_all_highresolu, normal_colors_all_highresolu = save_images(mesh, used_views)
            partialInstances,  original_cad_objs, original_cad_objs_confidence, \
                out_instance_face_confidence, original_cad_objs_fiterrors, original_cad_meshes, \
                original_cad_faceidxes, instance_idx_each_images = getPartialInstanceNew(mesh, result_masks, result_normals,
                                       normal_colors_all, extend_mask2instance, pixel_to_faces_all,
                                       pixel_zbuf_all, project_face_centers_all,
                                       pixel_to_faces_all_highresolu, zbuf_all_highresolu, normal_colors_all_highresolu,
                                       used_views, camera_locations, zbuf_min)
            real_mesh = mesh
            real_mesh_face_graph = nx.Graph()
            real_mesh_face_graph.add_nodes_from(list(range(len(real_mesh.faces))))
            real_mesh_face_graph.add_edges_from(mesh.face_adjacency)
            real_mesh_face_centers = real_mesh.vertices[real_mesh.faces].mean(axis=1)
            real_mesh_face_normals = real_mesh.face_normals
            new_partial_components = rematch_two_matches(mesh, [ins[0] for ins in partialInstances], real_mesh)
            new_partial_components = [ins[0] for ins in partialInstances]
            new_partial_newton_objects = [ins[-1] for ins in partialInstances]
            new_partial_newtonobjs = [ins[3] for ins in partialInstances]
            new_partial_instance_types = [ins[1] for ins in partialInstances]
            project_dis = np.array([[ 1- np.abs(np.dot(fn, ins.getnormal(fc))) for fc, fn in zip(real_mesh_face_centers, real_mesh_face_normals)] for ins in new_partial_newtonobjs ])
            potential_ins = project_dis.argmin(axis=0)

        with time_block("Third step: build graph between partial instances, and execute graph cut."):
            new_partial_components_sure = []
            new_partial_components_sure_labeles = np.zeros(len(real_mesh.faces))
            for idx in range(len(new_partial_components)):
                face_idx_comp = new_partial_components[idx]
                source_comp = new_partial_components[idx]
                new_partial_components_sure.append(source_comp)
                new_partial_components_sure_labeles[list(new_partial_components_sure[-1])] = idx + 1
            no_label_faces_idx = np.where(new_partial_components_sure_labeles == 0)[0]
            partialInstances, no_label_face_final_label = assign_no_label_faces_to_instances(real_mesh, no_label_faces_idx,
                                                                                             partialInstances, project_dis,
                                                                                             real_mesh_face_graph, out_instance_face_confidence)

            for f_idx in no_label_face_final_label.keys():
                ll = no_label_face_final_label[f_idx]
                new_partial_components_sure_labeles[f_idx] = ll + 1
            new_partial_components_sure_labeles -= 1


            
            components = build_instance_graph_new_max(mesh, partialInstances, instance_idx_each_images)
            
            

        with time_block("Fourth step: you konw the primitive stitching rely on a good segmentation boundary, so we need to do label smooth.  "):
            face_components = []
            label_components = []
            for comp in components:
                face_comp = [j for idx in comp for j in partialInstances[idx][0] ]
                other_part_mask = np.array([i for i in range(len(mesh.faces)) if i not in face_comp])
                other_part_comps =  list(nx.connected_components(real_mesh_face_graph.subgraph(other_part_mask)) )
                if len(other_part_comps) > 1:
                    removed_other_part = max(other_part_comps, key=len)
                    other_part_comps.remove(removed_other_part)
                    for o_face_idx_mask in other_part_comps:
                        if len(o_face_idx_mask) < 50:
                            face_comp = face_comp + list(o_face_idx_mask)
                face_components.append(face_comp)
                label_components.append( [partialInstances[idx][1] for idx in comp ][0])
            new_components = face_components
            # new_components = rematch_two_matches(mesh, face_components, real_mesh)
            new_components, label_components = process_final_connnected_components(mesh, new_components, [comp-1 for comp in label_components])
            patch_components = []
            for i in range(len(new_components)):
                patch_mesh = real_mesh.submesh([list(new_components[i])], repair=False)[0]
                patch_face_idx = new_components[i]
                c_patch_labels =label_components[i]
                c_patch, c_patch_face_idx, c_patch_labels = [patch_mesh], np.array(list(patch_face_idx)), c_patch_labels
                patch_components.append([c_patch, c_patch_face_idx, c_patch_labels])
                # render_simple_trimesh_select_faces(real_mesh, c_patch_face_idx)
            # render_all_patches(mesh, new_components)
            save_cache([real_mesh, patch_components], temp_path)

    else:
        real_mesh, patch_components = load_cache(temp_path)

    # render_all_patches(mesh, new_components)
    patch_comps = patch_components
    temp_path =  cfg.output_path + "/temp_scve" + os.path.basename(cfg.config_dir)
    if not os.path.exists(temp_path) or cfg.review:
        with time_block("Fifth step: ransac fit of each instance's primitives. "):
            print("ransac fit")
            ransac_objects = []
            new_patch_components = []
            for idx in range(len(patch_comps)):
                c_patch, c_patch_face_idx, c_patch_labels = patch_comps[idx]
                res, success_flag = run_with_timeout(c_patch[0].vertices, c_patch[0].vertex_normals, c_patch_labels, 0.1)
                if not success_flag:
                    continue
                ransac_objects.append(res)
                new_patch_components.append(patch_comps[idx])
            patch_comps = new_patch_components
            print("ransac fitting end")

        with time_block("Sixth step: get the final segmentation results. "):
            init_objs, init_comps, init_cad_objs = initial_get_fitted_params(ransac_objects, patch_comps, used_first_only = True)
            distance_to_all_init_objects = [[np.linalg.norm(obj.project(face_center) - face_center) for face_center in real_mesh.vertices[real_mesh.faces].mean(axis=1)]
                                             for obj in init_cad_objs]
            distance_to_all_init_objects = np.array(distance_to_all_init_objects)
            complete_labels = np.zeros(len(real_mesh.faces)).astype(np.int32) - 1
            for i in range(len(init_comps)):
                for idx in init_comps[i]:
                    if complete_labels[idx] == -1:
                        complete_labels[idx] = i
                    else:
                        if distance_to_all_init_objects[complete_labels[idx]][idx] > distance_to_all_init_objects[i][idx]:
                            complete_labels[idx] = i
        with time_block("Seventh step: You know six views cannot cover all the triangles, so we need to do label propagation."):
            no_label_idx = np.where(complete_labels==-1)[0]
            for i in no_label_idx:
                idx = distance_to_all_init_objects[:, i].argmin()
                complete_labels[i] = idx
            distance_metric = np.array(reassign_faces(real_mesh, list(range(len(real_mesh.faces))), init_cad_objs))
            real_mesh, complete_labels = filter_tiny_component(real_mesh, complete_labels, distance_metric)
        with time_block("Eight step: smooth the boundary label again (boundary label is so important). "):
            complete_labels = smooth_boundary_label(real_mesh, complete_labels, order= 3, smooth=3)
            complete_components = [np.where(complete_labels==ii)[0] for ii in range(complete_labels.max()+1)]
            complete_components, _, init_cad_objs = process_final_connnected_components_objs(real_mesh, complete_components, init_cad_objs, 
                                                                                             [cp[-1] for cp in patch_components])
            for ii in range(len(complete_components)):
                complete_labels[complete_components[ii]] =  ii
        with time_block("Ninth step: save the final segmentation results. "):
            abuntant_graph = build_intersection_graph(real_mesh, len(set(complete_labels)) + 1, complete_labels+1)
            new_labels, compact_cad_objs_label = remove_abuntant_objs(real_mesh, abuntant_graph, complete_labels, init_cad_objs)

            assert new_labels.max()+1 == len(init_cad_objs)
            bind_parallel_comps = None
            choose_count = len(set(new_labels))
            trimm_face_centers = real_mesh.vertices[real_mesh.faces].mean(axis=1)
            new_trimm = real_mesh
            components_labels = compact_cad_objs_label
            new_trimm_face_labels = new_labels
            save_cache([choose_count, trimm_face_centers, new_labels, new_trimm, components_labels,
                                new_trimm_face_labels, bind_parallel_comps], temp_path)
    else:
    ...
    with time_block("Tenth step: generate the step file. "):
        faces = fit_cad_faces(choose_count, trimm_face_centers, new_labels, new_trimm, 
                    components_labels,new_trimm_face_labels, bind_parallel_comps, cfg)
        save_step_file(faces, temp_path + ".step")
...
from mvdiffusion.data.abc_dataset import ABCDataset as MVDiffusionDataset
...
def log_validation(dataloader, vae, feature_extractor, image_encoder, unet, cfg: TrainingConfig, accelerator, weight_dtype, global_step, name, save_dir):
    ...
    # the pipeline decodes color-semantics + normal, so it needs the finetuned
    # mask_vae(+mlp) from finetune_vae_semantic and the normal_vae from finetune_vae_normal.
    from automl.AutoencoderKL import AutoencoderKL as CustomAutoencoderKL, ClassifyMLP
    mask_vae_ckpt = getattr(cfg, "mask_vae_ckpt", "./finetune_vae_semantic/vae_0.pth")
    mlp_ckpt = getattr(cfg, "mlp_ckpt", "./finetune_vae_semantic/mlp_0.pth")
    normal_vae_ckpt = getattr(cfg, "normal_vae_ckpt", "./finetune_vae_normal/vae_normal_0.pth")
    if all(os.path.exists(p) for p in (mask_vae_ckpt, mlp_ckpt, normal_vae_ckpt)):
        pipeline.mask_vae = CustomAutoencoderKL.from_pretrained(cfg.pretrained_model_name_or_path, subfolder="vae", revision=cfg.revision).to(torch.float32)
        pipeline.mask_vae.load_state_dict(torch.load(mask_vae_ckpt)); pipeline.mask_vae = pipeline.mask_vae.to(accelerator.device)
        pipeline.mlp = ClassifyMLP(128, hidden_size=256, num_classes=11)
        pipeline.mlp.load_state_dict(torch.load(mlp_ckpt)); pipeline.mlp = pipeline.mlp.to(accelerator.device)
        pipeline.normal_vae = CustomAutoencoderKL.from_pretrained(cfg.pretrained_model_name_or_path, subfolder="vae", revision=cfg.revision).to(torch.float32)
        pipeline.normal_vae.load_state_dict(torch.load(normal_vae_ckpt)); pipeline.normal_vae = pipeline.normal_vae.to(accelerator.device)
    else:
        logger.warning("skip validation: finetuned mask_vae/mlp/normal_vae checkpoints not found "
                       f"({mask_vae_ckpt}, {mlp_ckpt}, {normal_vae_ckpt}); train the VAEs first.")
        return
...
def main(
    ...
    generator = torch.Generator(device=accelerator.device).manual_seed(cfg.seed) if cfg.seed is not None else None
    ...
    unet = UNetMV2DConditionModel.from_pretrained_2d(cfg.pretrained_model_name_or_path, subfolder="unet", revision=cfg.revision, torch_dtype=torch.float16, **cfg.unet_from_pretrained_kwargs)
    ...
    if cfg.resume_from_checkpoint:
        ...
        if path is None:
            ...
            global_step = int(path.split("-")[1])   # restore step from checkpoint name (was hardcoded 0)

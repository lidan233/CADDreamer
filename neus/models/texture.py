...
class VolumeRadiance(nn.Module):
    ...
    def forward(self, features, dirs, *args):
        ...
        network_inp = torch.cat(
            [features.view(-1, features.shape[-1]), dirs_embd] + [arg.view(-1, arg.shape[-1]) for arg in args], dim=-1)
...
@models.register('volume-radiance-mask')
class VolumeRadianceMask(nn.Module):
    def __init__(self, config):
        super(VolumeRadiance, self).__init__()
        self.config = config
        self.n_dir_dims = self.config.get('n_dir_dims', 3)

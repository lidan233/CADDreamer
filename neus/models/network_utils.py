...
class VanillaFrequency(nn.Module):
    ...
    def update_step(self, epoch, global_step):
        if self.n_masking_step <= 0 or global_step is None:
            ...
            self.mask = (1. - torch.cos(
                math.pi * (global_step / self.n_masking_step * self.N_freqs - torch.arange(0, self.N_freqs)).clamp(0,
                                                                                                                   1))) / 2.
...
class ProgressiveBandHashGrid(nn.Module):
    def __init__(self, in_channels, config):
        ...
        self.start_level, self.start_step, self.update_steps = config['start_level'], config['start_step'], config[
            'update_steps']
...
class CompositeEncoding(nn.Module):
    ...
    def forward(self, x, *args):
        return self.encoding(x, *args) if not self.include_xyz else torch.cat(
...
def sphere_init_tcnn_network(n_input_dims, n_output_dims, config, network):
    ...
    assert data.shape[0] == (n_input_dims + n_output_dims) * config.n_neurons + (

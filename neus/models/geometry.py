...
class MarchingCubeHelper(nn.Module):
    ...
    def grid_vertices(self):
        if self.verts is None:
            x, y, z = torch.linspace(*self.points_range, self.resolution), torch.linspace(*self.points_range,
                                                                                          self.resolution), torch.linspace(
                *self.points_range, self.resolution)
...
class BaseImplicitGeometry(BaseModel):
    def __init__(self, config):
        ...
        if self.config.isosurface is not None:
            ...
            if self.config.isosurface.method == 'mc-torch':
                raise NotImplementedError(
                    "Please do not use mc-torch. It currently has some scaling issues I haven't fixed yet.")
    ...
    def isosurface(self):
        if self.config.isosurface is None:
            raise NotImplementedError
        mesh_coarse = self.isosurface_((-self.radius, -self.radius, -self.radius),
...
class VolumeDensity(BaseImplicitGeometry):
    def setup(self):
        ...
        self.n_semantic_dims = self.config.semantic_dim
        ...
        self.semantic_with_network = get_encoding_with_network(self.n_input_dims, self.n_semantic_dims,
                                                               self.config.xyz_encoding_config,
                                                               self.config.smlp_network_config)
    ...
    def forward(self, points):
        ...
        semantic_logits = self.semantic_with_network(points.view(-1, self.n_input_dims)).view(*points.shape[:-1],
                                                                                              self.n_semantic_dims).float()
        ...
        return density, feature, semantic_logits

    def forward_level(self, points):
        points = contract_to_unisphere(points, self.radius, self.contraction_type)
        density = self.encoding_with_network(points.reshape(-1, self.n_input_dims)).reshape(*points.shape[:-1],
    ...
    def update_step(self, epoch, global_step):
        ...
        update_module_step(self.semantic_with_network, epoch, global_step)
...
class VolumeSDF(BaseImplicitGeometry):
    def setup(self):
        ...
        semantic_network = get_mlp(encoding.n_output_dims, self.n_semantic_dims, self.config.smlp_network_config)
        self.semantic_network = semantic_network
        self.encoding, self.network = encoding, network
        self.semantic_encoding = get_encoding(3, self.config.xyz_encoding_config)
    ...
    def forward(self, points, with_grad=True, with_feature=True, with_laplace=False, with_semantic=False):
        with torch.inference_mode(
            ...
            with torch.set_grad_enabled(self.training or (with_grad and self.grad_type == 'analytic')):
                ...
                points = contract_to_unisphere(points, self.radius,
                                               self.contraction_type)  # points normalized to (0, 1)

                out = self.network(self.encoding(points.view(-1, 3))).view(*points.shape[:-1],
                                                                           self.n_output_dims).float()
                semantic_logits = self.semantic_network(self.semantic_encoding(points.view(-1, 3))).view(
                    *points.shape[:-1], self.n_semantic_dims).float()
                ...
                if with_grad:
                    if self.grad_type == 'analytic':
                        ...
                        points_d_sdf = self.network(self.encoding(points_d.view(-1, 3)))[..., 0].view(
                            *points.shape[:-1], 6).float()
                        grad = 0.5 * (points_d_sdf[..., 0::2] - points_d_sdf[..., 1::2]) / eps

                        if with_laplace:
                            laplace = (points_d_sdf[..., 0::2] + points_d_sdf[..., 1::2] - 2 * sdf[..., None]).sum(
                                -1) / (eps ** 2)
    ...
    def update_step(self, epoch, global_step):
        ...
        update_module_step(self.semantic_encoding, epoch, global_step)
        update_module_step(self.network, epoch, global_step)
        update_module_step(self.semantic_network, epoch, global_step)
        if self.grad_type == 'finite_difference':

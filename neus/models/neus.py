...
from nerfacc import ContractionType, OccupancyGrid, ray_marching, render_weight_from_density, render_weight_from_alpha, \
...
class NeuSModel(BaseModel):
    def setup(self):
        ...
        # self.texture_mask = models.make(self.config.texture.name, self.config.texture)
        ...
        if self.config.learned_background:
            ...
            # self.texture_mask_bg = models.make(self.config.texture_bg.name, self.config.texture_bg)
    ...
    def update_step(self, epoch, global_step):
        ...
        if self.training and self.config.grid_prune:
            ...
            if self.config.learned_background:
                self.occupancy_grid_bg.every_n_step(step=global_step, occ_eval_fn=occ_eval_fn_bg,
    ...
    def forward_bg_(self, rays):
        ...
        density, feature, semantic_logits = self.geometry_bg(positions)
        rgb = self.texture_bg(feature, t_dirs)

        weights = render_weight_from_density(t_starts, t_ends, density[..., None], ray_indices=ray_indices,
                                             n_rays=n_rays)
        ...
        comp_rgb_mask = accumulate_along_rays(weights, ray_indices, values=semantic_logits, n_rays=n_rays)

        out = {
            'comp_rgb': comp_rgb,
            'mask_rgb': comp_rgb_mask,
            'opacity': opacity,
            'depth': depth,
    ...
    def forward_(self, rays):
        ...
        if self.config.geometry.grad_type == 'finite_difference':
            sdf, sdf_grad, feature, semantic_logits, sdf_laplace = self.geometry(positions, with_grad=True,
                                                                                 with_feature=True, with_laplace=True,
                                                                                 with_semantic=True)
        else:
            sdf, sdf_grad, feature, semantic_logits = self.geometry(positions, with_grad=True, with_feature=True,
                                                                    with_semantic=True)
        ...
        comp_semantic_mask = accumulate_along_rays(weights, ray_indices, values=semantic_logits, n_rays=n_rays)
        ...
        out = {
            'comp_rgb': comp_rgb,
            'semantic_rgb': comp_semantic_mask,
        ...
        if self.config.learned_background:
            ...
            out_bg = {
                ...
                'semantic_rgb': self.background_semantic[None, :].expand(*comp_semantic_mask.shape),
                'num_samples': torch.zeros_like(out['num_samples']),
                'rays_valid': torch.zeros_like(out['rays_valid']),
                'comp_normal': torch.zeros_like(comp_normal)
            }

        out_full = {
            'comp_rgb': out['comp_rgb'] + out_bg['comp_rgb'] * (1.0 - out['opacity']),
            'semantic_rgb': out['semantic_rgb'] + out_bg['semantic_rgb'] * (1.0 - out['opacity']),
            'comp_normal': out['comp_normal'] + out_bg['comp_normal'] * (1.0 - out['opacity']),
    ...
    def export(self, export_config):
        ...
        if export_config.export_vertex_color:
            ...
            _, sdf_grad, feature, semantic_logits = chunk_batch(self.geometry, export_config.chunk_size, False,
                                                                mesh['v_pos'].to(self.rank), with_grad=True,
                                                                with_feature=True, with_semantic=True)
            normal = F.normalize(sdf_grad, p=2, dim=-1)
            rgb = self.texture(feature, -normal, normal)  # set the viewing directions to the normal to get "albedo"
            mask_labels = logits_2_label(semantic_logits)

            mesh['v_rgb'] = rgb.cpu()
            mesh['v_mask_rgb'] = mask_labels.cpu()
        return mesh

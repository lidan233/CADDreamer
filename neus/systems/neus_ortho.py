...
def cross_entropy_soft_labels(y_true, y_pred):
    epsilon = 1e-7
    y_pred = torch.clamp(y_pred, epsilon, 1 - epsilon)  # 使用torch.clamp来限制预测值的范围
    loss = -torch.sum(y_true * torch.log(y_pred), dim=-1)  # 计算交叉熵损失
...
class OrthoNeuSSystem(BaseSystem):
    ...
    def preprocess_data(self, batch, stage):
        ...
        if stage in ['train']:
            ...
            semantic_logits = self.dataset.all_semantics[index, y, x].view(-1, 1).to( self.rank)
            ...
            semantic_logits = self.dataset.all_semantics[index].view(-1, self.dataset.all_semantics.shape[-1]).to(self.rank)
        ...
        if stage in ['train']:
            ...
            self.model.background_semantic = torch.zeros((self.model.config.geometry.semantic_dim,), dtype=torch.float32, device=self.rank)
        else:
            self.model.background_color = torch.ones((3,), dtype=torch.float32, device=self.rank)
            self.model.background_semantic = torch.zeros((self.model.config.geometry.semantic_dim,), dtype=torch.float32, device=self.rank)
    ...
    def training_step(self, batch, batch_idx):
        ...
        class_weight = torch.ones(self.model.config.geometry.semantic_dim).to(batch['semantic_logits'].device)
        class_weight[8] = 0.05
        CrossEntropyLoss = nn.CrossEntropyLoss(weight = class_weight)
        ...
        ray_rgb = out['comp_rgb_full'][rgb_mask]
        supervised_rgb =  batch['rgb'][rgb_mask]
        erros_rgb_mse = F.mse_loss(ray_rgb, supervised_rgb, reduction='none')
        ...
        ray_labels = out['semantic_rgb_full'][torch.where(batch['semantic_logits']!=0)[0]]
        gt_labels = batch['semantic_logits'].to(torch.float32)[torch.where(batch['semantic_logits']!=0)]
        real_gt_label = gt_labels.reshape(-1).to(torch.uint8)
        if self.useedge:
            ray_labels = ray_labels[torch.where((real_gt_label<=6) | (real_gt_label ==8))]
            real_gt_label = real_gt_label[torch.where((real_gt_label<=6) | (real_gt_label ==8))]
        else:
            ray_labels = ray_labels[torch.where((real_gt_label<=6) )]
            real_gt_label = real_gt_label[torch.where((real_gt_label<=6) )]


        semantic_loss_fine =  crossentropy_loss(ray_labels, real_gt_label)
        # semantic_loss_fine = cross_entropy_soft_labels(ray_labels,  gt_labels)
        loss += semantic_loss_fine * 0.0000001
        self.log('train/loss_semantic_mse', semantic_loss_fine * 0.0000001, prog_bar=True, rank_zero_only=True)


        # normal_errors = 1 - F.cosine_similarity(out['comp_normal'][torch.where(batch['semantic_logits']!=8)[0]], batch['normal'][torch.where(batch['semantic_logits']!=8)[0]], dim=1)
        ...
        if self.config.system.loss.geo_aware:
            # tmask = mask[torch.where(batch['semantic_logits'] != 8)[0]]
            # tcosines = cosines[torch.where(batch['semantic_logits'] != 8)[0]]
            # tview_weights = view_weights[torch.where(batch['semantic_logits'] != 8)[0]]
            # normal_errors = normal_errors * torch.exp(tcosines.abs()) / torch.exp(tcosines.abs()).sum()
            tmask = mask
            tcosines = cosines
            tview_weights = view_weights
            normal_errors = normal_errors * torch.exp(tcosines.abs()) / torch.exp(tcosines.abs()).sum()

            loss_normal = ranking_loss(normal_errors[tmask],
                                       penalize_ratio=self.config.system.loss.normal_p_ratio,
                                       extra_weights=tview_weights[tmask],
                                       type='sum')
        else:
    ...
    def export(self):
        ...
        mm = trimesh.Trimesh(mesh['v_pos'], mesh['t_pos_idx'], process=False)
        save_cache_dill([mm, mesh['v_mask_rgb']], os.path.join(self.config.dataset.scene, str(self.useedge)+'0.obj'))
        mm.export( os.path.join(self.config.dataset.scene, str(self.useedge)+'_mm.obj'))
        self.save_mesh(

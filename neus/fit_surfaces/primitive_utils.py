...
# from third_party.geomfitty.geomfitty import geom3d
# from third_party.geomfitty.geomfitty._descriptor import Direction, Position
# from third_party.geomfitty.geomfitty._util import distance_line_point
# from third_party.geomfitty.geomfitty.fit3d import _check_input
...
#             u, v = np.meshgrid(u, v)
#             points = parametric_equation(u.flatten(), v.flatten())
#             faces = trimesh.convex.convex_hull(points)
#             mesh = trimesh.Trimesh(vertices=points, faces=faces)
#             return mesh
#         self.torus_cc = generate_mesh()


#     def distance_to_point(self, point):
#         distance, _, _ = trimesh.proximity.closest_point(self.torus_cc, [point])
#         return distance

#     def __repr__(self):
#         return f"Torus (center={self.center}, major_radius={self.major_radius}, minor_radius={self.minor_radius}"
# def fittorus(points, weights=None, initial_guess: Cone = None):
#     _check_input(points, weights)

#     initial_guess = [
#         Torus(np.array([1.0, 0, 0]),  np.zeros(3)),
#         Torus(np.array([0, 1.0, 0]), np.zeros(3)),
#         Torus(np.array([0, 0, 1.0]), np.zeros(3)),
#     ]
...
#     return apex, axis, theta, err, failure, results.fun

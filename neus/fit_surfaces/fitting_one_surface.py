...
# from third_party.geomfitty.geomfitty._util import distance_line_point
# from fit_surfaces.fitting_utils import project_to_plane, visualize_basic_mesh
# from fit_surfaces.layers import PositionalEncoding, ResBlock, SirenLayer, SirenWithResblock
# from fit_surfaces.primitive_forward import Fit
# from fit_surfaces.fitting_utils import get_rng
# from fit_surfaces.fitting_utils import sample_inr_mesh


# def process_one_surface(label, points, labels, cfg, device, current_label):
...
#     if current_label !=  5:
#         print("# ========================= fitting basic primitives =======================")
#         recon_basic_shapes = fit_basic_primitives(in_points, current_label)
#         current_type = label_map[current_label]
#     else:
#         print("# ========================== fitting inrs=====================")
...
#         pred_info['err_list'] = recon_basic_shapes["plane_err_list"]

#     elif current_type == "Sphere":  # sphere
#         pred_mesh = visualize_basic_mesh(
...
#         pred_info['err_list'] = recon_basic_shapes["sphere_err_list"]

#     elif current_type == "Cylinder":  # cylinder
#         pred_mesh = visualize_basic_mesh(
...
#         pred_info['err_list'] = recon_basic_shapes["cylinder_err_list"]

#     elif current_type == "Cone":  # cone
#         pred_mesh = visualize_basic_mesh(
...
#         # pred_info['err_list'] = recon_basic_shapes["cone_err_list"]
#     elif current_type == "Torus":  # torus
#         out = {}
#         out["mesh"] = None
#         out["info"] = {}
#         out['info']['type'] = 'torus'
...
# def fit_basic_primitives(pts, current_label):
#     """
...
#         plane_err_list = torch.linalg.norm(new_points - pts, dim=-1)
...
#         recon_basic_shapes["plane_err_list"] = plane_err_list.tolist()
...
#         sphere_err_list = (torch.linalg.norm(pts - center, dim=-1) - radius).abs()
...
#         recon_basic_shapes["sphere_err_list"] = sphere_err_list.data.cpu().numpy().tolist()
...
#         recon_basic_shapes["cylinder_err_list"] = cylinder_err_list.tolist()
...
#             # recon_basic_shapes["cone_err_list"] = error_list.tolist()
#     elif current_label == 4:
#         # torus = fitting.fit_torus(
#         #     pts.cpu().numpy(),
#         #     normals=np.zeros_like(pts.cpu().numpy()),
#         #     weights=np.ones_like(pts.cpu().numpy())[:, :1],
#         #     ids=None,
#         # )
...
def process_one_surface(label, points, labels, cfg, device, current_label):
    ...
    if current_type == "Plane":  # plane
        ...
        triangles = np.asarray(pred_mesh.triangles)
        faces = np.column_stack((np.full(len(triangles), 3), triangles))
        faces = faces.flatten()
        pred_mesh = pv.PolyData(vertices, faces)

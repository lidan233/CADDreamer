...
def main(inputcfg=None):
    ...
    group.add_argument('--train', default=True, action='store_true')
    group.add_argument('--validate', action='store_true')
    group.add_argument('--test', action='store_true')
    group.add_argument('--predict', action='store_true')
    group.add_argument('--export', action='store_true') # TODO: a separate export action

    parser.add_argument('--exp_dir', default='./neus/exp')
    parser.add_argument('--runs_dir', default='./neus/runs')
    parser.add_argument('--verbose', action='store_true', help='if true, set logging level to DEBUG')

    args, extras = parser.parse_known_args()

    if inputcfg is not None:
        for key in inputcfg.keys():
            args.__setattr__(key, inputcfg[key])


    # set CUDA_VISIBLE_DEVICES then import psytorch-lightning
    os.environ['CUDA_DEVICE_ORDER'] = 'PCI_BUS_ID'
    os.environ['CUDA_VISIBLE_DEVICES'] = inputcfg['gpu']
    n_gpus = len(inputcfg['gpu'].split(','))
    ...
    if inputcfg is not None:
        extras.extend(inputcfg['dataset'])

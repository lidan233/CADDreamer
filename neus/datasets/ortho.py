...
#         normal = normal[:, :, :3]
#
#         color_mask = np.array(PIL.Image.open(os.path.join(root_dir,test_object, 'masked_colors/rgb_000_%s.png'%( view))).resize(imSize, resample =PIL.Image.Resampling.NEAREST))[:, :, 3]
...
#
#         tcolor = PIL.Image.open(os.path.join(root_dir,test_object, 'rgb_000_%s.png'%( view))).resize(imSize, resample =PIL.Image.Resampling.NEAREST)
#         color_mask1 = np.array(tcolor).astype(np.float32)
#         color_mask1 = np.abs(color_mask1 - color_mask1[0, 0].reshape(1,1,-1)).mean(axis=-1) > 30
#         color_mask = color_mask & color_mask1
#
#
#
...
#
#         # from utils.util import load_cache_dill
#         # t_normals, t_masks = load_cache_dill("/home/lidan/GeoWizard/test1.cache")
#         # t_normals = [np.array(Image.fromarray((t_normal * 255).astype(np.uint8)).resize(imSize, resample =PIL.Image.Resampling.NEAREST))[:, :, :3] for t_normal in t_normals]
#         # t_masks = [np.array(Image.fromarray((t_mask * 255).astype(np.uint8)).resize(imSize, resample =PIL.Image.Resampling.NEAREST))[:, :, :3] for t_mask in t_masks]
#         # boundary_idx  = np.all(t_masks[0] == np.array([0, 255,0]).reshape(1,1,-1), axis=2)
#         # range_sets = []
#         # for color, pdis in zip(mask_colors[::-1], abs_dis[::-1]):
#         #     range_set = pdis[boundary_idx]
#         #     range_sets.append(range_set)
#         #     print("distance to " +str(color), "min dis:" , np.min(range_set), 'mean dis', np.mean(range_set), 'max dis:', np.max(range_set))
#         #
#
#         mask = semantic_mask!=0
#         color_mask = semantic_mask!=0
#
...
#         all_gts, all_masks = connect_components_analysis_single(all_masks, temp_semantics)
#         all_color_masks = all_masks
#
#
#
#     # for gti in range(len(all_gts)):
#     #     gt = all_gts[gti]
#     #     c_normal = all_normals[gti]
#     #     all_normals[gti][np.where((gt == 8) & (c_normal.sum(axis=-1) == 0))] = 1
#

...
def save_images(object_file: str, viewidx: int, color_size=14, line_thickness=2) -> None:
    global VIEWS
    global EXTRA_VIEWS
    reset_scene()
...
def render_hdr_image(object_file:str, viewidx: int, color_size=14, line_thickness=2):
    global VIEWS
    global EXTRA_VIEWS
    reset_scene()
...
if __name__ == "__main__":
    ...
    if True :
        try:
            local_path = "/media/lida/softwares/Wonder3D/test_outputs/cropsize-256-cfg1.0/890/False_mm.obj"
            save_images(local_path, args.view)
            # render_hdr_image(local_path, args.view)
    ...
    bpy.data.cameras[0].type = "ORTHO"
    bpy.data.cameras[0].ortho_scale = args.ortho_scale
    bpy.ops.wm.save_mainfile(filepath='/media/lida/softwares/lidan2.blend')

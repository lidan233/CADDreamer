...
import numpy as np
from render_codes.render_cad_dataset import *
...
def get_random_material():
    texture_dir = "/media/lida/softwares/haven/textures"
    textfiles = os.listdir(texture_dir)

    textfiles = [file for file in textfiles if 'metal' in file]
    ...
    hdrfiles = [file for file in hdrfiles if 'adams_place' in file]
    ...
    bproc.world.set_world_background_hdr_img(used_hdr_img)
    global HDR_FILE
...
def save_images(object_file: str, viewidx: int, input_data, color_size=14, line_thickness=2) -> None:
    ...
    for location in camera_locations:
        ...
        light_name = f'light_{len(bpy.data.lights)}'
        _light = bproc.types.Light(name=light_name, light_type='SUN')
        light_obj = bpy.data.lights[light_name]
        light_obj.use_shadow = False
        light_obj.specular_factor = 1.0
        light_obj.energy = 3
        light_object = bpy.data.objects[light_name]
        light_object.location = _location
        light_object.rotation_euler = _rotation
        light_object.rotation_euler[0] += math.radians(90)  # Add 90 degrees in radians
        ...
        light_object.parent = cam_empty
        light_constraint.target = cam_empty
        light_constraint.owner_space = 'LOCAL'
        light_object.parent = _camera
    ...
    for j in range(len(VIEWS)):
        ...
        cv2.imwrite('{}/{}/depth_{}.png'.format(args.output_folder, object_uid, view), depth_map, [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
def render_hdr_image(object_file:str, viewidx: int, input_data, color_size=14, line_thickness=2):
    global VIEWS
    global EXTRA_VIEWS
    reset_scene()
    ...
    for j in range(len(VIEWS)):
        ...
        color_map = data['colors'][index]
        view = f"{viewidx:03d}" + VIEWS[j]
        old_img = io.imread('{}/{}/rgb_{}.png'.format(args.output_folder, object_uid, view))
        valid_mask = np.where(old_img[:, :, -1] == 255)
        color_map[valid_mask] = old_img[:,:,:3][valid_mask]
        Image.fromarray(color_map.astype(np.uint8)).save(
            '{}/{}/rgbbackground_{}.png'.format(args.output_folder, object_uid, view), "png", quality=100)
...
if __name__ == "__main__":
    ...
    start_i = time.time()
    save_dir = "/media/lida/systems/dataset/cad_image_overfit4"
    DATA_ROOT = "/media/lida/systems/dataset/deepcad"
    ...
    paths = all_data["train"]


    path = paths[args.sizeidx]
    result = process_cad_data(RAW_DATA, path, type="deepcad", tolerance=0.0001, color_size=12)
    local_path = os.path.join(args.object_path, path.split('/')[1] + '.obj')

    if result['out_mesh'] is not None :
        try:
            result['out_mesh'].export(local_path)
            save_images(local_path, args.view, result)
            render_hdr_image(local_path, args.view, result)
    ...
    bpy.utils.register_class(QMMSettings)
    bpy.types.Scene.diffuse_bool = bpy.props.PointerProperty(type=QMMSettings)
    bpy.data.cameras[0].type = "ORTHO"
    bpy.data.cameras[0].ortho_scale = args.ortho_scale
    bpy.ops.wm.save_mainfile(filepath='/media/lida/softwares/lidan1.blend')

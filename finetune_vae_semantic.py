...
from mvdiffusion.data.abc_dataset_vae import ABCDataset as MVDiffusionDataset
...
def main(args):
    ...
    if True:
        ...
        classify = ClassifyMLP(128, hidden_size=256, num_classes=11)   # must match inference (test_mvdiffusion_seq.py)

        # train the VAE decoder + the primitive-type MLP classifier
        vae.requires_grad_(False)        # freeze all, then unfreeze decoder only (below)
        classify.requires_grad_(True)
        vae_trainable_params = []
        for name, param in vae.named_parameters():
            if 'decoder' in name:
                param.requires_grad = True
                vae_trainable_params.append(param)
        for name, param in classify.named_parameters():
            param.requires_grad = True
            vae_trainable_params.append(param)
        ...
        vae, classify, optimizer, train_dataloader, validation_dataloader, lr_scheduler = accelerator.prepare(
            vae, classify, optimizer, train_dataloader, validation_dataloader, lr_scheduler
        )
        weight_dtype = torch.float32
        vae.to(accelerator.device, dtype=weight_dtype)
        classify.to(accelerator.device, dtype=weight_dtype)
        ...
        for epoch in range(first_epoch, num_train_epochs):
            for step, batch in enumerate(train_dataloader):
                with accelerator.accumulate(vae) ,  accelerator.accumulate(classify) :
                # with accelerator.accumulate([vae, classify]):
                    imgs_out = batch['imgs_out_mask']
                    ...
                    images = ((imgs_out * 2.0 - 1.0).clamp(-1, 1).permute(0, 2, 3, 1) / 2 + 0.5).clamp(0, 1)
                    images = images.detach()
                    images = (images * 255).round().to(torch.uint8 )

                    class_mask = torch.zeros(images.shape[0:3]).to(imgs_out.device)
                    for color_i in range(len(mask_colors)):
                        color = mask_colors[color_i]
                        idx = torch.where(torch.all(images == color.reshape(1, 1, 1, -1).to(imgs_out.device), dim=-1))
                        class_mask[idx] = color_i + 1

                    predict_label = classify(pred_images_feature.permute(0,2,3,1))
                    real_label = class_mask

                    loss_mask = F.cross_entropy(predict_label.reshape(-1, predict_label.shape[-1]),
                                           real_label.reshape(-1).to(torch.long))

                    loss = F.mse_loss(pred_images.float(), (imgs_out * 2.0 - 1.0).clamp(-1, 1).float(), reduction="mean")
                    loss1 = F.l1_loss(pred_images.float(), (imgs_out * 2.0 - 1.0).clamp(-1, 1).float(), reduction="mean")
                    loss = loss + loss1 + loss_mask
                    print("sum loss:", loss, " mask loss:", loss_mask, "l1 rgb loss:", loss1)
                ...
                if global_step % args.validation_steps == 0:
                    vae.eval(); classify.eval()
                    val_losses = []
                    with torch.no_grad():
                        for vbatch in validation_dataloader:
                            v_out = rearrange(vbatch['imgs_out_mask'], "B Nv C H W -> (B Nv) C H W").to(weight_dtype)
                            v_lat = vae.encode(v_out * 2.0 - 1.0).latent_dist.sample()
                            v_pred, v_feat = vae.decode(v_lat, return_dict=False)[0]
                            v_imgs = (v_out * 2.0 - 1.0).clamp(-1, 1)
                            v_cls = torch.zeros(((v_out * 2.0 - 1.0).clamp(-1, 1).permute(0, 2, 3, 1).shape[:3])).to(v_out.device)
                            v_pix = (((v_out * 2.0 - 1.0).clamp(-1, 1).permute(0, 2, 3, 1) / 2 + 0.5).clamp(0, 1) * 255).round().to(torch.uint8)
                            for color_i in range(len(mask_colors)):
                                idx = torch.where(torch.all(v_pix == mask_colors[color_i].reshape(1, 1, 1, -1).to(v_out.device), dim=-1))
                                v_cls[idx] = color_i + 1
                            v_predlabel = classify(v_feat.permute(0, 2, 3, 1))
                            v_lmask = F.cross_entropy(v_predlabel.reshape(-1, v_predlabel.shape[-1]), v_cls.reshape(-1).to(torch.long))
                            v_l = F.mse_loss(v_pred.float(), v_imgs.float()) + F.l1_loss(v_pred.float(), v_imgs.float()) + v_lmask
                ...
                if global_step % 500 == 0:
                    ...
                    model_path = args.output_dir + f'/mlp_{epoch}.pth'
                    torch.save(classify.state_dict(), model_path)

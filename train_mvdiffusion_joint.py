...
def log_validation(dataloader, vae, feature_extractor, image_encoder, unet, cfg: TrainingConfig, accelerator, weight_dtype, global_step, name, save_dir):
    ...
    # joint pipeline decodes color-semantics + normal too, so it needs the finetuned
    # mask_vae(+mlp) from finetune_vae and the normal_vae from finetune_vae_normal.
    ...
    if all(os.path.exists(p) for p in (mask_vae_ckpt, mlp_ckpt, normal_vae_ckpt)):
        ...
        logger.warning("skip joint validation: finetuned mask_vae/mlp/normal_vae checkpoints not found "

...
from utils.graphcut import build_instance_graph_new_max, build_instance_graph_new
...
# import concurrent.futures
# def run_with_timeout(vertices, normals, c_ins_label):
#        with concurrent.futures.ThreadPoolExecutor() as executor:
#         future = executor.submit(run_fit, vertices, normals, c_ins_label)
#         try:
...
"""
--config_dir ./test_outputs/cropsize-256-cfg1.0/0_0deepcad --review False
"""
if __name__=='__main__':
    ...
    try:
        if not os.path.exists(temp_path) or cfg.review:
            with time_block("First step: load the prediction masks and normals."):
            ...
            with time_block("Second step: get the partial instances."):
            ...
            with time_block("Third step: get the sure instances."):
                ...
                components = build_instance_graph_new(mesh, partialInstances, instance_idx_each_images)
                
                

            with time_block("Fourth step: get the face components."):
                face_components = []
                label_components = []
        ...
        if not os.path.exists(temp_path) or cfg.review:
                    ...
                    for idx in init_comps[i]:
                        if complete_labels[idx] == -1:
                            ...
                            if distance_to_all_init_objects[complete_labels[idx]][idx] > distance_to_all_init_objects[i][idx]:
                                complete_labels[idx] = i
            with time_block("Seventh step: get the final segmentation results. "):
            ...
            with time_block("Eight step: smooth the boundary label. "):
            ...
            with time_block("Ninth step: remove the abuntant objects. "):
        ...
        print(e)
        print("Error in stage 2 segmentation. Please send me an email. If I have time, I will check it. ")
        exit()
...
class ABCDataset(Dataset):
    def __init__(self,
        ...
        h5_files = sorted(f for f in os.listdir(self.root_dir) if f.endswith(('.h5', '.h5py')))
        self.file_objs = [h5py.File(os.path.join(self.root_dir, file), 'r') for file in h5_files]
        file_objs_sizes = [self.file_objs[i][key].shape[0] for i in range(len(self.file_objs)) for key in
                           self.file_objs[i].keys() if 'normals_000_back' in key]
        self.file_objs_keys = [key.split('normals')[0] for i in range(len(self.file_objs)) for key in
                               self.file_objs[i].keys() if 'normals_000_back' in key]
        self.file_objs_input = [self.file_objs[i] for i in range(len(self.file_objs)) for key in
                                self.file_objs[i].keys() if 'normals_000_back' in key]
        ...
        # train / test are SEPARATE h5 dirs (save_h5/train , save_h5/test).
        # Use ALL objects in whichever root_dir was given; no tail split.
    ...
    def __getitem_mix__(self, index, debug_object=None):
        ...
        return  {
                ...
                'imgs_out_normal': img_tensors_normal_out,
    ...
    def __getitem_joint__(self, index, debug_object=None):
        ...
        return {
                ...
                'azimuths_deg': torch.rad2deg(azimuths),
                'imgs_in': img_tensors_in,
                'imgs_out_normal': normal_tensors_out,
                'camera_embeddings': camera_embeddings,
                'task_embeddings': normal_task_embeddings
            }

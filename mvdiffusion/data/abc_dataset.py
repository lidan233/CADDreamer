...
class ABCDataset(Dataset):
    def __init__(self,
        ...
        self.file_objs = [h5py.File(os.path.join(self.root_dir, file), 'r') for file in tqdm(h5_files)]
        ...
        # train / test are now SEPARATE h5 dirs (save_h5/train , save_h5/test).
    ...
    def __getitem_mix__(self, index, debug_object=None, choose_normal_only=False, choose_mask_only=False):
        ...
        # NOTE: training does not use the mesh/name/rotate (meta_data is never read by the
        # train scripts). The h5 packed by step3/MergeH5 does not store name/vertices/faces,
        # so use lightweight placeholders instead of loading a mesh.
        current_name = str(current_prefix)
        current_mesh = ""        # collate-safe placeholder (training never reads the mesh)
        current_rotate = 0

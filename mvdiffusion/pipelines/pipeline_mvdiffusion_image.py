...
#             other_image, image_features = self.mask_vae.decode(latents.to(torch.float32) / self.vae.config.scaling_factor, return_dict=False)[0]
#             label_flags = self.mlp(image_features.permute(0, 2, 3, 1))
#             normal_images, normal_image_features = self.normal_vae.decode(latents.to(torch.float32) / self.vae.config.scaling_factor, return_dict=False)[0]
...
#         normal_images = self.image_processor.postprocess(normal_images, output_type=output_type, do_denormalize=do_denormalize)

#         # import numpy as np
#         # tt = (image[0].cpu().permute(1, 2, 0).numpy().astype(np.float32) * 255).astype(np.uint8)
#         # from skimage import io
#         # io.imshow(tt)
#         # io.show()
#         if not return_dict:
#             return (image, has_nsfw_concept, label_flags)

#         out =  StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept)
#         out.labels_flags = label_flags
#         out.normal_images = normal_images
#         return out
